...
export function SoloScreen({ state, question, titleById, records, savedRecordId, inputKind, command, settings = () => {}, finish, exit }: {
  ...
  return <SoloFrame onMenu={() => command({ type: "pause" })} onContinue={phase.kind === "reveal" ? () => command({ type: "continue" }) : undefined}>
...
export function SoloFeedbackScreen({ value, choose, toggleReason, setNote, submit, pending, error, exit }: {
  ...
  return <SoloFrame onMenu={exit}><section className="difficulty-feedback-stage solo-feedback" aria-labelledby="solo-feedback-title"><div className="stage-label">Фидбэк о вопросе</div><p className="feedback-difficulty">Сложность: {difficultyLabel(value.round.difficulty)}</p><h2 id="solo-feedback-title">{choice ? "Хотите пожаловаться на вопрос?" : "Что не так с вопросом?"}</h2><div className="feedback-tag-board" role="list">{choice ? <><button type="button" className={value.feedbackCursor === 0 ? "feedback-tag feedback-tag--cursor" : "feedback-tag"} disabled={pending} onClick={() => choose(true)}><strong>Да</strong></button><button type="button" className={value.feedbackCursor === 1 ? "feedback-tag feedback-tag--cursor" : "feedback-tag"} disabled={pending} onClick={() => choose(false)}><strong>Нет, продолжить</strong></button></> : <>{reasons.map(([reason, label], index) => <button type="button" className={value.feedbackCursor === index ? "feedback-tag feedback-tag--cursor" : "feedback-tag"} disabled={pending} onClick={() => toggleReason(reason)} key={reason}><strong>{label}</strong>{value.complaintReasons.includes(reason) && <span className="feedback-tag-state">✓</span>}</button>)}<button type="button" className={value.feedbackCursor === 7 ? "feedback-tag feedback-tag--cursor" : "feedback-tag"} disabled={pending || !canSubmitComplaint} onClick={submit}><strong>{pending ? "Сохраняем…" : "Готово"}</strong></button></>}</div>{!choice && <label className="feedback-note">Заметка (необязательно, до 500 символов)<textarea value={value.complaintNote} maxLength={500} disabled={pending} onChange={(event) => setNote(event.target.value)} /></label>}<p className={error ? "feedback-status feedback-status--error" : "feedback-status"} aria-live="polite">{pending ? "Сохраняем фидбэк на сервере…" : error ? error : choice ? "Нажмите «Да» или «Нет, продолжить»" : "Выберите хотя бы одну причину или добавьте заметку, затем нажмите «Готово»"}</p></section></SoloFrame>;
}
